...
namespace Utilz
{
    ...
    public sealed class Logger
    {
        ...
        public static async Task<string> ReadAllLogsIntoStringAsync()
        {
            ...
            output += await ReadOneLogIntoStringAsync(Logger.AppExceptionLogFilename).ConfigureAwait(false);
            output += await ReadOneLogIntoStringAsync(Logger.BackgroundCancelledLogFilename).ConfigureAwait(false);
            output += await ReadOneLogIntoStringAsync(Logger.BackgroundLogFilename).ConfigureAwait(false);
            output += await ReadOneLogIntoStringAsync(Logger.FileErrorLogFilename).ConfigureAwait(false);
            output += await ReadOneLogIntoStringAsync(Logger.ForegroundLogFilename).ConfigureAwait(false);
            output += await ReadOneLogIntoStringAsync(Logger.PersistentDataLogFilename).ConfigureAwait(false);
            return output;
        }
        public static async Task<string> ReadOneLogIntoStringAsync(string filename)
        {
            ...
            output += await ReadAsync(Logger.AppExceptionLogFilename);
            output += Environment.NewLine;
            return output;
        }
